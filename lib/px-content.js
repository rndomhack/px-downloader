...
class PxContent {
    ...
    getFilename(options) {
        ...
        const _dirname = this.replaceMacro(options.dirname);
        const _filename = this.replaceMacro(options.filename);

        if (options.hasOwnProperty("index")) {
            const _pagename = this.replacePageMacro(options.pagename, options.index);

            filename = `${_dirname}/${_pagename}.${options.ext}`.replace(/\/+/g, "/");
        } else {
            filename = `${_dirname}/${_filename}.${options.ext}`.replace(/\/+/g, "/");
        }
    ...
    downloadIllust(options) {
        ...
        return this.util.fetch({ url: imageUrl, type: "blob", init: { credentials: "include", referrer: location.href } }).then(imageBlob => {
            ...
            return this.download({
                blob: imageBlob,
                filename: this.getFilename({ dirname: options.dirname, filename: options.filename, ext: this.getExt(imageBlob) }),
        ...
        return this.util.fetch({ url: pageUrl, type: "text", init: { credentials: "include", referrer: location.href } }).then(pageText => {
            ...
            imageBlobs.forEach((imageBlob, index) => {
                promise = promise.then(() => {
                    return this.download({
                        blob: imageBlob,
                        filename: this.getFilename({ dirname: options.dirname, filename: options.filename, pagename: options.pagename, index: index, ext: this.getExt(imageBlob) }),
                        conflictAction: options.conflictAction
                    });
                });
            });
    ...
    downloadUgoiraZip(options) {
        ...
        return this.util.fetch({ url: zipUrl, type: "arraybuffer", init: { credentials: "include", referrer: location.href } }).then(arrayBuffer => {
            ...
            return this.download({
                blob: zipBlob,
                filename: this.getFilename({ dirname: options.dirname, filename: options.filename, ext: this.getExt(zipBlob) }),
                conflictAction: options.conflictAction
            });
        });
    }
    ...
    downloadNovel(options) {
        ...
        return this.util.fetch({ url: pageUrl, type: "text", init: { credentials: "include", referrer: location.href } }).then(pageText => {
            ...
            return this.download({
                blob: textBlob,
                filename: this.getFilename({ dirname: options.dirname, filename: options.filename, ext: this.getExt(textBlob) }),
                conflictAction: options.conflictAction
            });
        });
    }
