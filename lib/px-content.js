...
class PxContent {
    ...
    downloadMultiple(options) {
        ...
        return this.util.fetch({ url: firstPageUrl, type: "text", init: { credentials: "include", referrer: location.href } }).then(firstPageText => {
    ...
    addButton() {
        ...
        const listener = () => {
            ...
            this.downloadPixiv().then(() => {
                this.button.textContent = "Redownload";
            }).catch(err => {
